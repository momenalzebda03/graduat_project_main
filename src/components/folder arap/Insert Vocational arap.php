...
if (empty($Name_vocational) || empty($Job_vocational) || empty($Specialization_vocational) || empty($Experience_vocational) || empty($Experience_vocational1) || empty($Experience_vocational2) || empty($Experience_vocational3) || empty($String_Skill_vocational) || empty($Skill_vocational) || empty($Skill_vocational1) || empty($Skill_vocational2) || empty($Password_vocational) || empty($Image_vocational)) {
    ...
    $statement->execute([$Name_vocational, $Job_vocational, $Specialization_vocational, $Experience_vocational, $Experience_vocational1, $Experience_vocational2, $Experience_vocational3, $String_Skill_vocational, $Skill_vocational, $Skill_vocational1, $Skill_vocational2, $Password_vocational, $Image_vocational]);
    if ($query >= 0) {
        $url = "http://localhost:8080/page_vocational_arap";
        header("Location: " . $url);
    }
}
